    ...
    def fetch_historical_data(self, period="2y") -> pd.DataFrame:
        ...
        for t in self.tickers:
            ...
            if not s.empty and len(s) > (days * 0.65):
                all_series[t] = s
                print(f"  [CACHE] {t}: Loaded from LOCAL CACHE ({len(s)} rows)")
                continue

            print(f"  [NET] {t}: Cache miss. Fetching from external APIs...")
            try:
                ...
                print(f"  [OK] {t}: Downloaded & Cached successfully.")

            except Exception as e:
                print(f"  [ERR] {t}: ALL APIs FAILED -> {str(e)}")
