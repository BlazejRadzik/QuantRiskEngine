...
st.markdown("""
    ...
    /* Karty doradcy */
    .advisor-card { border: 1px solid #d1d5da; border-radius: 8px; padding: 20px; background-color: #ffffff; margin-bottom: 20px; }
    </style>
...
# ==========================================
    ...
    if st.button("Wykonaj obliczenia", type="primary"):
        with st.spinner("Pobieranie danych i obliczanie modeli..."):
            try:
                res = requests.get("https://quantriskengine.onrender.com/v1/portfolio/risk", params={"tickers": tickers_input}, timeout=20)
                ...
                st.error(f"Błąd API: {e}. Sprawdź logi serwera Render!")
...
# ==========================================
    ...
    advisor_data = {
        "Konserwatywny": {
            ...
            "reason": "Wybrane spółki charakteryzują się niskim współczynnikiem Beta i stabilnymi przepływami pieniężnymi (Cash Flow). W okresach bessy chronią kapitał dzięki niskiej korelacji z rynkiem technologicznym.",
        ...
        "Zrównoważony": {
            ...
            "reason": "Złoty środek między wzrostem (Growth) a wartością (Value). Portfel zdywersyfikowany między sektor Tech a sektor finansowy, zapewniający ekspozycję na liderów gospodarki.",
        ...
        "Agresywny": {
            ...
            "reason": "Skupienie na spółkach o wysokiej dynamice wzrostu i klastrowaniu zmienności. Model GARCH(1,1) wykazuje tutaj potencjał na wysokie stopy zwrotu kosztem zwiększonego ryzyka ogona (Fat Tails).",
    ...
    with c3:
        # Wyliczenie przykładowych ilości (zakładamy ceny rynkowe dla przykładu)
        ...
        for i, t in enumerate(data['tickers']):
            ...
            cols[i].metric(t, f"{data['weights'][i]*100}%", f"{int(share)} {currency}")
    ...
    # Generowanie przykładowej ścieżki (Random Walk) dla wizualizacji
    np.random.seed(42)
    days = 252
...
# ==========================================
    ...
    st.markdown("Pobierz dane wejściowe i raporty walidacyjne.")
